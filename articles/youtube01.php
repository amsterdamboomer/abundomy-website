<?php 
    ...
    // =========================================================================
    // Establish absolute system directory path roots
    ...
    $jsonPath = $projectRoot . "/../json/youtube01.json";
    $fallbackTitle = "Banks Versus One Coin Per Hour"; 
    $fallbackDesc = "How fast banks extract wealth from society.";
    $fallbackBrand = "Abundomy";

    if (file_exists($jsonPath)) {
        $jsonData = json_decode(file_get_contents($jsonPath), true);
        // tx_01 isolates the Video Title, tx_02 isolates the Subtitle brief description
    ...
    // Explicit dynamic key-phrases matching this specific presentation topic
    $pageKeywords = "Banking Extraction, Wealth Distribution, One Coin Per Hour, Banking System Flaws, Compound Interest";
...
<div id="capture-section" class="selectable-text <?php echo $isRTL ? 'rtl-mode' : ''; ?>" <?php echo $isRTL ? 'dir="rtl"' : 'dir="ltr"'; ?>>
    ...
    <div class="article-header-row">
        <div class="author-wrap">
            <table class="author-table">
              ...
              <tr>
                <td class='imgtext' id='tx_03'>24 March 2026</td>
              </tr>
            </table>
        </div>
    </div>
    <!-- ========================================= -->
    <div class="short-video-wrapper" style="position: relative; background: #000; overflow: hidden; border-radius: 12px; border: 2px solid var(--text, #916B01); box-shadow: 0 8px 24px rgba(0,0,0,0.5);">
        
        <!-- 1. Environment-Aware Custom Thumbnail -->
        <img src="<?php echo $baseHref; ?>img/Thumbnail Banks vs One Coin Per Hour 540x960.jpg" 
             alt="Video Preview" 
             style="width: 100%; height: 100%; object-fit: cover; opacity: 0.75;" />
        
        <!-- 2. Overlay Interface Layer -->
        <div style="position: absolute; inset: 0; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; text-align: center; gap: 15px; background: rgba(0, 0, 0, 0.35);">
            ...
            <!-- Centered Interactive Play Button Element -->
            <a href="https://youtube.com/shorts/4TCKFP8mJsI" 
               target="_blank" 
    ...
    <!-- Centered Download Interface Section -->
    <div class="download-section-wrap">
        <a href="<?php echo $baseHref; ?>download/OneCoinHDemo.xlsx" download="OneCoinHDemo.xlsx">
            <button type="button" class="login-button" id="tx_05">Download Excel Data</button>
        </a>
    </div>
...
<script>
    ...
    async function loadArticle() {
        ...
        try {
            ...
            const response = await fetch('<?php echo $baseHref; ?>json/youtube01.json');
